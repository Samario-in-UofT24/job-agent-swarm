"""Follow-up Writer — LangGraph Node 6. See AGENT_CONTRACT.md § Node 6."""
...
__all__ = ["followup_writer"]

T = TypeVar("T", bound=BaseModel)


class FollowupEmailModel(BaseModel):
...
def followup_writer(state: dict[str, Any]) -> dict[str, Any]:
    """Writes followup_email and increments email_revision_count."""
    required = ("parsed_job", "parsed_resume", "match_result", "customized_resume")
    missing = [k for k in required if k not in state]
    if missing:
        raise ValueError(f"missing required state keys: {missing}")

    prev = int(state.get("email_revision_count") or 0)
    payload = {
        ...
        "email_revision_count": prev,
    }

    system = _load_prompt("followup_prompt.txt")
    result = _invoke_structured(FollowupEmailModel, system, payload)
    return {
        "followup_email": result.model_dump(),
        "email_revision_count": prev + 1,
    }
